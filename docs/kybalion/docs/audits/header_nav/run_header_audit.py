...
def build_markdown(rows: list[dict[str, str]], html_files: list[Path]) -> str:
	...
	lines.append("## Canonical Controls Index")
	lines.append("")
	lines.append("| Canonical Name | Quick Role | Primary Surface |")
	lines.append("|---|---|---|")
	lines.append("| `main_menu_button_1` | Global main-menu trigger + panel contract | Shared header/menu wrapper |")
	lines.append("| `doc_menu_link_1` | Standard navigation/document menu item contract | `#menuPanel` and `#menuSessionsFlyout` |")
	lines.append("| `sessions_trigger_1` | Sessions submenu trigger + flyout contract | Documents section in `#menuPanel` |")
	lines.append("| `auth_menu_link_1` | Auth state menu links (signed-out/signed-in) | Navigation section in `#menuPanel` |")
	lines.append("| `docs_admin_action_button_1` | Docs admin upload/create header actions | Docs `.header-actions` cluster |")
