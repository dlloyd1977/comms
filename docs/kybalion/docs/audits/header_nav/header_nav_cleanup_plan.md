...
| `doc_menu_link_1` | `.menu-panel .menu-link` (standard links such as Home, Kybalion Home, Reader, Document Library, General, Templates, Session links, and admin-only docs links) | Full-width outlined link button; accent text/border on white panel background; consistent `menu-link` token usage with optional state tokens (`is-hidden`, `admin-only`) | Single-action navigation item inside the Main Menu panel/flyout; supports standard click navigation and keyboard focus traversal | Inside `#menuPanel` and `#menuSessionsFlyout` only; grouped under `Navigation` and `Documents` sections in canonical order | Label consistency, ordering contract, role-based visibility rules (auth links/admin links), and keyboard compatibility with menu focus-loop + roving-focus contracts |
| `sessions_trigger_1` | `#menuSessionsBtn.menu-link.menu-sessions-trigger` + `#menuSessionsFlyout.menu-sessions-flyout` | Same outlined menu-link visual language as other panel links; nested flyout uses same link styling for session entries | Toggle-style submenu trigger that opens/closes sessions flyout and manages focus into/out of session link list | In `Documents` section between `General` and `Templates`; directly controls immediate sibling flyout container | ARIA linkage (`aria-controls="menuSessionsFlyout"`, `aria-expanded` sync), Escape close/focus return, first-item focus on open, Tab/Shift+Tab containment, and Arrow/Home/End roving-focus behavior |
| `auth_menu_link_1` | `#menuAuthLink.menu-link`, `#menuChangePasswordLink.menu-link`, `#menuSignOutLink.menu-link` | Uses shared `menu-link` visual style; hidden-state variants use `is-hidden` token without changing base style | Auth state switcher inside menu (visitor vs signed-in controls) including sign-in redirect, password link, and sign-out action | Navigation section of `#menuPanel`, directly below core global links (`Home`, `Kybalion Home`, `Reader`, `Document Library`) | Runtime auth parity contract: exactly one visitor entry visible when signed out; change-password + sign-out visible when signed in; admin/member visibility remains role-aware |
| `docs_admin_action_button_1` | `#headerUploadBtn.button.primary`, `#headerNewFolderBtn.button.secondary`, `#uploadInput.file-input-hidden` | Upload uses primary CTA style; New Folder uses secondary style; input remains visually hidden file picker control | Admin-only document actions for upload/create workflows in docs header | Docs page header actions cluster (`.header-actions`) adjacent to `.menu-wrapper` | Visibility and click flow controlled by admin/auth state; upload button maps to hidden file input trigger; actions must remain keyboard focusable when visible |
| `reader_profile_button_1` | `#headerProfileBtn.button.secondary`, `#headerSignOutBtn.button.secondary` (reader header controls) | Secondary outlined button style matching reader header control palette | Reader account controls for profile modal and sign-out | Reader controls area near auth/user display, separate from menu panel links | Profile opens modal workflow; sign-out clears auth state; both controls must respect signed-in visibility and keyboard activation contracts |
